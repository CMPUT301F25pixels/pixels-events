...
public class LotteryNotificationService {
    ...
    public void checkAndNotifyRegistrationDeadlines() {
        ...
        String currentUserId = prefs.getString(KEY_PROFILE_ID, "0");
     ...
     */
    private void checkEventForNotification(Event event, String currentUserId) {
        ...
        try {
            ...
            if (registrationEndDate != null && registrationEndDate.before(now)) {
                ...
                databaseHandler.getWaitingList(eventId,
                    waitingList -> {
                        if (waitingList != null && waitingList.isEntrantOnList(currentUserId)) {
     ...
     */
    private boolean hasBeenNotified(String eventId) {
        SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
     ...
     */
    private void markAsNotified(String eventId) {
        SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
        String notifiedEvents = prefs.getString(KEY_NOTIFIED_EVENTS, "");
